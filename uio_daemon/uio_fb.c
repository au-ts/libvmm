...
int main() {
    ...
    int fb_fp = open("/dev/fb0", O_WRONLY);
    if (fb_fp == -1) {
        perror("Error opening file");
        return 10;
    }

    void *fbmap = mmap(0, FB_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fb_fp, 0);
    ...
    while (read(uio_fp, &read_value, sizeof(uint32_t)) == sizeof(uint32_t)) {
        ...
        printf("Copying from map0 to fbmap\n");
