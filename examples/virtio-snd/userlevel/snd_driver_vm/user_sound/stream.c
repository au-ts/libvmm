...
{
    sddf_snd_pcm_data_t *response = queue_front(stream->consumed_frames);
    if (!response) {
        return false;
    }
                      ...
                      sddf_snd_pcm_data_ring_t *pcm_responses)
{
    ...
    stream->consumed_frames = queue_create(sizeof(sddf_snd_pcm_data_t), PCM_QUEUE_SIZE);
