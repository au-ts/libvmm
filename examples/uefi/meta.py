...
qemu_ramfb_paddr = 0x600_0000          # size 0x200000
qemu_fw_cfg_dma_paddr = 0x800_0000     # size 0x1000
hw_net_rings_paddr = 0x801_0000        # size 0x10000
virtio_blk_metadata_paddr = 0x802_0000 # size 0x10000
virtio_blk_requests_paddr = 0x803_0000 # size 0x10000
guest_vapic_paddr = 0x804_0000         # size 0x1000
guest_apic_access_paddr = 0x804_1000   # size 0x1000
guest_low_ram_paddr = 0x1000_0000      # size 0x6E000000
guest_high_ram_paddr = 0x1_0000_0000   # size 0x200000000
@dataclass
class Board:
    name: str
...
def x86_virtio_gpu_passthrough(vmm_client0, vm_client0):
    # This comes from what we tell OVMF the PCI MMIO aperature is, because the firmware will try to relocate the
...
def generate(sdf_file: str, output_dir: str, dtb: Optional[DeviceTree], client_dtb: Optional[DeviceTree], pci_base: int):
    ...
    guest_ram_mr = MemoryRegion(sdf, name="guest_ram", size=0x6E000000, paddr=guest_low_ram_paddr)
    ...
    guest_ram_high_mr = MemoryRegion(sdf, name="guest_ram_high", size=0x200000000, paddr=guest_high_ram_paddr)
