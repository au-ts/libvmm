...
{    
    ...
    if ((storage_fd = open("/root/storage", O_CREAT, O_RDWR)) < 0) {
    ...
    while (!blk_req_queue_empty(&h)) {
        ...
        switch(req_code) {
            case READ_BLOCKS:
                ...
                int bytes_read = read(storage_fd, (void *)data_phys_to_virt(addr), req_count * blocksize);
                if (bytes_read < 0) {
            ...
            case WRITE_BLOCKS:
                lseek(storage_fd, req_block_number * blocksize, SEEK_SET);
                int bytes_written = write(storage_fd, (void *)data_phys_to_virt(addr), req_count * blocksize);
                if (bytes_written < 0) {
