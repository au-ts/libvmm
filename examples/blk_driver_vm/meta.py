...
def generate(sdf_file: str, output_dir: str, dtb: DeviceTree, driver_vm_dtb: DeviceTree, pmu_event_ids: List[int],):
    ...
    blk_driver_vmm_pd = ProtectionDomain("blk_driver_vm", "blk_driver_vmm.elf", priority=106, cpu=0)
    blk_driver_vm = VirtualMachine("linux", [VirtualMachine.Vcpu(id=0)], priority=105)
    blk_driver_vmm = Vmm(sdf, blk_driver_vmm_pd, blk_driver_vm, driver_vm_dtb, one_to_one_ram=True)


    timer_system.add_client(blk_driver_vmm_pd)

    blk_driver_vmm.add_virtio_mmio_console(guest_serial_node, serial_system)

    for device_path in board.passthrough:
        node = dtb.node(device_path)
        assert node is not None
        blk_driver_vmm.add_passthrough_device(node)
