...
#define UART_MODE_CHRL_REF 		(0b0 << 0)   // Clock source select (clock source is xil_uart_ref_clk)
#define UART_MODE_CLKS_REF_8 	(0b1 << 0)   // Clock source select (clock source is xil_uart_ref_clk/8)
...
#define serial_puts(str_buffer) xil_uart_puts(1,(const int8_t *)str_buffer)
