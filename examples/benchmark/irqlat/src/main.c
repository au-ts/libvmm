...
static inline void pmu_print_header()
{
    for (size_t i = 0; i < pmu_used_counters; i++)
    {
        ...
        const char *priv = priv_code == 0xc8 ? "_el2" : priv_code == 0x08 ? "_el1+2"
                                                                          : "_el1";
        char buf[COL_SIZE];
...
void my_timer_handler(unsigned id)
{
    curr_cycles = pmu_cycle_get();
    uint64_t diff = curr_cycles - last_cycles;
    printf("diff: %lu\n", diff);
    printf("curr_cycles: %lu\n", curr_cycles);
    printf("last_cycles: %lu\n", last_cycles);
    sample_count += 1;
...
void timer_handler(unsigned id)
{
    timer_disable();
    curr_time = timer_get();
    irqlat_samples[sample_count] = (uint64_t)((float)(curr_time - next_tick) / TIMER_FREQ * 1000000000.0);

    for (volatile size_t i = 0; i < 15; i++)
        ;
    curr_time = timer_get();
    irqlat_end_samples[sample_count] = (uint64_t)((float)(curr_time - next_tick) / TIMER_FREQ * 1000000000.0);
...
void main(void)
{
    ...
    printf("Finished setting timer IRQ\n");
