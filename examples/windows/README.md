...
In addition to the dependencies listed in the root `README.md`, the following
dependencies are needed:

1. A custom kernel that enables Intel APICv operation, since APICv is not officially
...
or build the SDK from source yourself, see [docs/custom_microkit.md](docs/custom_microkit.md)
for more details.

2. sdfgen (for generating the System Description File with a metaprogram).
...
For the first boot, we recommend that you use the debug Microkit configuration
to spot any issues. For subsequent boot, use the Microkit release
...
The crash looks like this, note that you will only see the crash message under
Microkit debug configuration:
```
...
that `VIRTIO_NET_PCI_IRQ` and `VIRTIO_BLK_PCI_IRQ` match this output too.
