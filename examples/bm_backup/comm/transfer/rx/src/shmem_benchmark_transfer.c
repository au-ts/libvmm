...
void shmem_benchmark_transfer_handler(unsigned _id) {

    size_t cycle_length = transfer_ctrl->buf_size > transfer_ctrl->transfer_size ?
        transfer_ctrl->transfer_size : transfer_ctrl->buf_size;

    volatile unsigned long *ptr = SHMEM_DATA_BASE;
    for (size_t j = 0; j < cycle_length/sizeof(*ptr); j++) {
        (void) *ptr++;
    }

    transfer_ctrl->received = true;
    shmem_clear_event();
}

void shmem_benchmark_transfer() {

    timer_wait(TIME_MS(100));
    printf("rx: Inter-VM Transfer Benchmark\n");

    shmem_init();

    irq_set_handler(SHMEM_IRQ, shmem_benchmark_transfer_handler);
    ...
    while(true) {
        ...
        while (!transfer_ctrl->start_polling) {
        ...
        const size_t transfer_size = transfer_ctrl->transfer_size;
        const size_t buf_size = transfer_ctrl->buf_size;
        // printf("polling 0x%lx:0x%lx\n", transfer_size, buf_size);
        size_t multiple_cycles = transfer_size/buf_size > 0;
        size_t num_cycles = multiple_cycles ? transfer_size/buf_size : 1;
        size_t cycle_length = multiple_cycles ? buf_size : transfer_size;

        for (sample_count = 0; sample_count < NUM_SAMPLES + NUM_WARMUPS; sample_count++) {
            // pmu_setup(counter_start, sample_events_size - counter_start);
            for (size_t i = 0; i < num_cycles; i++) {
                while(!transfer_ctrl->sent);
                transfer_ctrl->sent = false;
                shmem_benchmark_transfer_handler(0);
            }
            // printf("next sample\n");
            // pmu_sample(j);
        }

        // printf("next transfer\n");
        
        // if (sample_count >= NUM_SAMPLES) {
        //     sample_count = 0;
        //     counter_start += pmu_num_counters();
        //     if (counter_start >= sample_events_size) {
        //         print_samples();
        //         counter_start = 0;
        //     }
        // }
