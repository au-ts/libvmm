...
void pl011_init(uint64_t gpa) {
    bool success = virq_register(0, 33, &pl011_irq_ack, NULL);
    assert(success);
    success = fault_register_vm_exception_handler(gpa, 0x1000, pl011_access, NULL);
    assert(success);
}
