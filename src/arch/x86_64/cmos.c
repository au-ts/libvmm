...
bool emulate_cmos_access(seL4_VCPUContext *vctx, uint16_t port_addr, bool is_read, ioport_access_width_t access_width)
{
    ...
    if (port_addr == 0x70) {
        if (is_read) {
            vctx->eax = microkit_x86_ioport_read_8(CMOS_ADDR_PORT_ID, 0x70);
        } else {
            cmos_regs.selected_reg = vctx->eax & 0x7f;
            microkit_x86_ioport_write_8(CMOS_ADDR_PORT_ID, 0x70, vctx->eax);
            LOG_CMOS("selecting register 0x%x\n", cmos_regs.selected_reg);
        }
    } else if (port_addr == 0x71) {
        if (is_read) {
            if (cmos_regs.selected_reg > 0xd) {
                vctx->eax = 0;
            } else {
                vctx->eax = microkit_x86_ioport_read_8(CMOS_DATA_PORT_ID, 0x71);
            }
        } else {
            microkit_x86_ioport_write_8(CMOS_DATA_PORT_ID, 0x71, vctx->eax);
        }
    }
