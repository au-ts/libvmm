...
static bool map_page(uint64_t paging_objects_start_gpa, void *pml4, uint64_t target_gpa, uint64_t *n_pt_created)
{
    ...
    if (pml4_entry & PTE_PRESENT_BIT) {
        ...
        *n_pt_created = *n_pt_created + 1;
        ...
        void *pdpt = gpa_to_hva(pdpt_gpa, PAGE_SIZE_4K);
        if (!pdpt) {
    ...
    if (pdpt_entry & PTE_PRESENT_BIT) {
        ...
        *n_pt_created = *n_pt_created + 1;
        ...
        void *pd = gpa_to_hva(pd_gpa, PAGE_SIZE_4K);
        if (!pd) {
                                                 ...
                                                 uint64_t ident_map_end_gpa, uint64_t *ret_paging_objs_bytes)
{
    ...
    void *pml4 = gpa_to_hva(pml4_gpa, PAGE_SIZE_4K);
    if (!pml4) {
                        ...
                        linux_x86_setup_ret_t *ret)
{
    ...
    uint64_t ram_end_gpa = ram_start_gpa + guest_ram_regions[0].size;
    ...
    void *kernel_load_dest = gpa_to_hva(kernel_load_gpa, kernel_load_size);
    assert(kernel_load_dest); // free memory already checked above
    ...
    void *initrd_dest = gpa_to_hva(initrd_gpa, initrd_size);
    if (!initrd_dest) {
        LOG_VMM_ERR("linux_setup_images(): out of memory for initrd, needed %lu\n", initrd_size);
        return false;
    }
    ...
    void *cmdline_dest = gpa_to_hva(cmdline_gpa, cmdline_len + 1);
    if (!cmdline_dest) {
        LOG_VMM_ERR("linux_setup_images(): out of memory for cmdline, needed %lu\n", cmdline_len);
        return false;
    }
    ...
    void *zero_page_dest = gpa_to_hva(zero_page_gpa, PAGE_SIZE_4K);
    if (!zero_page_dest) {
        LOG_VMM_ERR("linux_setup_images(): out of memory for zero page, needed %d\n", PAGE_SIZE_4K);
        return false;
    }
    ...
    uint64_t *gdt = gpa_to_hva(gdt_gpa, PAGE_SIZE_4K);
    if (!gdt) {
        LOG_VMM_ERR("linux_setup_images(): out of memory for GDT, needed %d\n", PAGE_SIZE_4K);
        return false;
    }
    gdt[0] = 0;
