...
*/
bool handle_sip(size_t vcpu_id, seL4_UserContext *regs, size_t fn_number)
{
...
bool handle_smc(size_t vcpu_id, uint32_t hsr)
{
    ...
    switch (service) {
        case SMC_CALL_STD_SERVICE:
            ...
            LOG_VMM_ERR("Failed to handle SMC standard service call %lu\n", fn_number);
            break;
        case SMC_CALL_SIP_SERVICE:
            /* If the seL4 configuration does not allow us to forward SMC calls, then we
             * simply drop through to the default case in the switch statement. */
#if defined(CONFIG_ALLOW_SMC_CALLS)
#if defined(DEBUG_SMC)
            LOG_VMM("Handling SiP call with function number: 0x%lx\n", fn_number);
#endif
            if (!handle_sip(vcpu_id, &regs, fn_number)) {
                LOG_VMM_ERR("Failed to handle SMC SiP service call %lu\n", fn_number);
                return false;
            } else {
                return true;
            }
#endif
        default:
            // @ivanv: print out a string of what service that is unknown
