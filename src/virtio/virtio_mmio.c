...
static bool handle_virtio_mmio_reg_read(uint64_t fault_addr, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    switch (offset) {
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_DEVICE_FEATURES, REG_VIRTIO_MMIO_DEVICE_FEATURES_SEL):
            err = emul_handler->funs->get_device_features(emul_handler, &reg);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_STATUS, REG_VIRTIO_MMIO_QUEUE_DESC_LOW):
            err = handle_virtio_mmio_get_status_flag(emul_handler, &reg);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_CONFIG, REG_VIRTIO_MMIO_CONFIG + 0x100):
            err = emul_handler->funs->get_device_config(emul_handler, offset, &reg);
    ...
    uint32_t mask = fault_get_data_mask(fault_addr, fsr);
    int fault_err = fault_advance(regs, fault_addr, fsr, reg & mask);
    assert(!fault_err);
...
static bool handle_virtio_mmio_reg_write(uint64_t fault_addr, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    switch (offset) {
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_DRIVER_FEATURES, REG_VIRTIO_MMIO_DRIVER_FEATURES_SEL):
            err = emul_handler->funs->set_driver_features(emul_handler, data);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_QUEUE_NOTIFY, REG_VIRTIO_MMIO_INTERRUPT_STATUS):
            err = emul_handler->funs->queue_notify(emul_handler);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_STATUS, REG_VIRTIO_MMIO_QUEUE_DESC_LOW):
            err = handle_virtio_mmio_set_status_flag(emul_handler, data);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_CONFIG, REG_VIRTIO_MMIO_CONFIG + 0x100):
            err = emul_handler->funs->set_device_config(emul_handler, offset, data);
            break;
        default:
