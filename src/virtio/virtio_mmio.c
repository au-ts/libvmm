...
static virtio_mmio_emul_handler_t *get_mmio_emul_handler_by_address(uint64_t addr)
{
    switch (addr)
    {
    case REG_RANGE(VIRTIO_NET_ADDRESS_START, VIRTIO_NET_ADDRESS_END):
        return get_virtio_net_mmio_emul_handler();
    // case REG_RANGE(VIRTIO_GPU_ADDRESS_START, VIRTIO_GPU_ADDRESS_END):
    //     return get_virtio_gpu_mmio_emul_handler();
    default:
        return NULL;
    }
}
...
static struct vring *get_current_vring_by_handler(virtio_mmio_emul_handler_t *mmio_emul_handler)
{
    assert(mmio_emul_handler->data.QueueSel < VIRTIO_MMIO_NET_NUM_VIRTQUEUE);
    return &mmio_emul_handler->vqs[mmio_emul_handler->data.QueueSel].vring;
}
...
*/
static int handle_virtio_mmio_get_status_flag(virtio_mmio_emul_handler_t *mmio_emul_handler, uint32_t *retreg)
{
    *retreg = mmio_emul_handler->data.Status;
    // printf("\"%s\"|VIRTIO MMIO|INFO: Status is 0x%x.\n", sel4cp_name, mmio_emul_handler->data.Status);
    return 1;
}

static int handle_virtio_mmio_set_status_flag(virtio_mmio_emul_handler_t *mmio_emul_handler, uint32_t reg)
{
    int success = 1;

    // we only care about the new status
    mmio_emul_handler->data.Status &= reg;
    reg ^= mmio_emul_handler->data.Status;
    // printf("VIRTIO MMIO|INFO: set status flag 0x%x.\n", reg);

    switch (reg) {
        case VIRTIO_CONFIG_S_RESET:
            mmio_emul_handler->data.Status = 0;
            mmio_emul_handler->funs->device_reset(mmio_emul_handler);
            break;

        case VIRTIO_CONFIG_S_ACKNOWLEDGE:
            // are we following the initialization protocol?
            if (mmio_emul_handler->data.Status == 0) {
                mmio_emul_handler->data.Status |= VIRTIO_CONFIG_S_ACKNOWLEDGE;
        ...
        case VIRTIO_CONFIG_S_DRIVER:
            // are we following the initialization protocol?
            if (mmio_emul_handler->data.Status && VIRTIO_CONFIG_S_ACKNOWLEDGE) {
                mmio_emul_handler->data.Status |= VIRTIO_CONFIG_S_DRIVER;
                // nothing to do from our side (as the virtio backend).
            }
            break;
        ...
        case VIRTIO_CONFIG_S_FEATURES_OK:
            // are we following the initialization protocol?
            if (mmio_emul_handler->data.Status && VIRTIO_CONFIG_S_DRIVER) {
                // are features OK?
                mmio_emul_handler->data.Status |= (mmio_emul_handler->data.features_happy ? VIRTIO_CONFIG_S_FEATURES_OK : 0);
            }
            break;

        case VIRTIO_CONFIG_S_DRIVER_OK:
            mmio_emul_handler->data.Status |= VIRTIO_CONFIG_S_DRIVER_OK;
...
static bool handle_virtio_mmio_reg_read(size_t vcpu_id, uint64_t fault_addr, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    virtio_mmio_emul_handler_t *mmio_emul_handler = get_mmio_emul_handler_by_address(fault_addr);
    assert(mmio_emul_handler);
    ...
    switch (offset) {
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_QUEUE_READY, REG_VIRTIO_MMIO_QUEUE_NOTIFY):
            if (mmio_emul_handler->data.QueueSel < VIRTIO_MMIO_NET_NUM_VIRTQUEUE) {
                reg = mmio_emul_handler->vqs[mmio_emul_handler->data.QueueSel].ready;
            }
            // simply ignore wrong queue number
            break;

        case REG_RANGE(REG_VIRTIO_MMIO_INTERRUPT_STATUS, REG_VIRTIO_MMIO_INTERRUPT_ACK):
            reg = mmio_emul_handler->data.InterruptStatus;
            break;

        case REG_RANGE(REG_VIRTIO_MMIO_STATUS, REG_VIRTIO_MMIO_QUEUE_DESC_LOW):
            success = handle_virtio_mmio_get_status_flag(mmio_emul_handler, &reg);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_CONFIG_GENERATION, REG_VIRTIO_MMIO_CONFIG):
             ...
             * here as a reminder. */
            // reg = mmio_emul_handler->data.ConfigGeneration;
            break;

        case REG_RANGE(REG_VIRTIO_MMIO_CONFIG, REG_VIRTIO_MMIO_CONFIG + 0x100):
            success = mmio_emul_handler->funs->get_device_config(mmio_emul_handler, offset, &reg);
...
static bool handle_virtio_mmio_reg_write(size_t vcpu_id, uint64_t fault_addr, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    switch (offset) {
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_QUEUE_READY, REG_VIRTIO_MMIO_QUEUE_NOTIFY):
            if (data == 0x1) {
                mmio_emul_handler->vqs[mmio_emul_handler->data.QueueSel].ready = true;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_QUEUE_NOTIFY, REG_VIRTIO_MMIO_INTERRUPT_STATUS):
            success = mmio_emul_handler->funs->queue_notify(mmio_emul_handler);
            break;

        case REG_RANGE(REG_VIRTIO_MMIO_INTERRUPT_ACK, REG_VIRTIO_MMIO_STATUS):
            mmio_emul_handler->data.InterruptStatus &= ~data;
            break;

        case REG_RANGE(REG_VIRTIO_MMIO_STATUS, REG_VIRTIO_MMIO_QUEUE_DESC_LOW):
            // printf("DATA IS 0x%x\n", data);
            success = handle_virtio_mmio_set_status_flag(mmio_emul_handler, data);
            break;
        ...
        case REG_RANGE(REG_VIRTIO_MMIO_CONFIG, REG_VIRTIO_MMIO_CONFIG + 0x100):
            success = mmio_emul_handler->funs->set_device_config(mmio_emul_handler, offset, data);
            break;
        default:
...
bool virtio_mmio_handle_fault(size_t vcpu_id, uint64_t fault_addr, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    if (!get_mmio_emul_handler_by_address(fault_addr)) {
